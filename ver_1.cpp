...
void InitAll()
{
	...
	for (int j = 0; j < point_num; j++)
	{
		...
		for (int ik = 0; ik < point_num; ik++)//匹配到有关系的点的位置
		{
			if (p[ik] == 1)
			{
				...
				if (point_color[j] == point_color[ik]) { flags++; conf[j] = 1; }
			}
		}
	}
	for (int i = 0; i < point_num; i++)
	{
		if (conf[i] == 1)
		{
			conf[f_number_point] = i; posf[i] = f_number_point++;
		}
	}
	f_number = flags / 2;
	MinofAll = f_number;
...
void Findmove() {
	...
	for (int ik = 0; ik<f_number_point; ik++) {
		int i = conf[ik];
...
void MakeMove()
{
	...
	for (int i = 0; i < point_num; i++)
	{
		if (p_node[i] == 1)
		{
			...
			if (point_color[i] == old_color)
			{
				conf[posf[i]] = conf[f_number_point];
				posf[conf[f_number_point]] = posf[i];
				f_number_point--;
			}
			else if(point_color[i] == new_color)
			{
				conf[f_number_point] = i;
				posf[i] = f_number_point++;
			}
		}
	}
